...
CONFIG = {
    ...
    'stop_loss_percentage': 0.005,  # เปอร์เซ็นต์ขาดทุนเริ่มต้น (ปรับได้ตามกลยุทธ์)
    ...
    'min_volatility_threshold': 0.005,  # ความผันผวนขั้นต่ำที่ยอมรับ
    'liquidity_threshold': 1000000,  # ปริมาณการซื้อขายขั้นต่ำ (หน่วย USDT)
...
UnifiedQuantumTrader.predict = predict
UnifiedQuantumTrader.train = train
UnifiedQuantumTrader.evolve = evolve
UnifiedQuantumTrader.adversarial_train = adversarial_train
...
class MultiMarketEnv(gym.Env):
    ...
    def get_observation(self):
        dyn_gen = DynamicIndicatorGenerator(self.trader.evogan, self.trader.gnnPamself.multi_tf_data)
        observations = []
        for symbol in self.symbols:
